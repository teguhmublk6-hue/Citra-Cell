...
import type { PPOBWifiFormValues, PPOBWifi } from '@/lib/types';
...
export default function PPOBWifiForm({ onTransactionComplete, onDone }: PPOBWifiFormProps) {
  ...
  const form = useForm<PPOBWifiFormValues>({
    resolver: zodResolver(PPOBWifiFormSchema),
    defaultValues: {
        sourcePPOBAccountId: '',
        customerName: '',
  ...
  const onSubmit = (values: PPOBWifiFormValues) => {
    onTransactionComplete(() => proceedWithTransaction(values));
  };
  
  const proceedWithTransaction = useCallback(async (values: PPOBWifiFormValues, force = false): Promise<any> => {
    ...
    if (!force) {
        const transactionsRef = collection(firestore, 'ppobWifi');
        const todayStart = startOfDay(new Date());
        const q = query(transactionsRef, where('date', '>=', Timestamp.fromDate(todayStart)));

        try {
            const querySnapshot = await getDocs(q);
            const todaysTransactions = querySnapshot.docs.map(doc => doc.data() as PPOBWifi);
    ...
    try {
        const auditDocRef = await addDocumentNonBlocking(collection(firestore, 'ppobWifi'), {
            date: now,
            ...
            switch (paymentMethod) {
                case 'Tunai':
                    ...
                    transaction.set(creditTunaiRef, {
                         kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Tagihan Wifi`, account: values.customerName, date: nowISO, amount: totalAmount, balanceBefore: currentLaciBalance, balanceAfter: currentLaciBalance + totalAmount, category: 'ppob_wifi_payment', deviceName, auditId
                    });
                    break;
                case 'Transfer':
                    ...
                    transaction.set(creditTransferRef, {
                        kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Tagihan Wifi`, account: values.customerName, date: nowISO, amount: totalAmount, balanceBefore: currentPaymentBalance, balanceAfter: currentPaymentBalance + totalAmount, category: 'ppob_wifi_payment', deviceName, auditId
                    });
                    break;
                case 'Split':
                    if (!laciAccountRef || !laciDoc || !laciDoc.exists()) throw new Error("Akun Laci tidak ditemukan.");
                    ...
                    transaction.set(creditSplitTunaiRef, {
                         kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Tunai Tagihan Wifi`, account: values.customerName, date: nowISO, amount: splitTunaiAmount, balanceBefore: currentLaciSplitBalance, balanceAfter: currentLaciSplitBalance + splitTunaiAmount, category: 'ppob_wifi_payment', deviceName, auditId
                    });
                    ...
                    transaction.set(creditSplitTransferRef, {
                        kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Transfer Tagihan Wifi`, account: values.customerName, date: nowISO, amount: splitTransferAmount, balanceBefore: currentPaymentSplitBalance, balanceAfter: currentPaymentSplitBalance + splitTransferAmount, category: 'ppob_wifi_payment', deviceName, auditId
                    });
                    break;
            }
        });
            ...
            {currentStep === 2 && (
              <div className="space-y-4">
                 <FormField control={form.control} name="customerName" render={({ field }) => (
                    <FormItem>
                        <FormLabel>Nama / ID Pelanggan</FormLabel>
                        <FormControl>
                            <Input 
                                placeholder="Masukkan nama/ID..." {...field} ref={inputRefs.customerName}
