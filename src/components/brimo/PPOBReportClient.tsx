...
export default function PPOBReportClient({ onDone }: PPOBReportProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    const tableData = reports.map((report, index) => [
        index + 1,
        report.serviceName,
        getAccountLabel(report.sourcePPOBAccountId),
        report.destination,
    ...
    autoTable(doc, {
        head: [['No', 'Layanan', 'Akun PPOB', 'Tujuan', 'Harga Modal', 'Harga Jual', 'Oleh']],
        body: tableData,
        startY: 30,
        theme: 'grid',
