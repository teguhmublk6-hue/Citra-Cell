...
interface PPOBPaketTelponReviewProps {
    formData: PPOBPaketTelponFormValues;
    onConfirm: () => void;
    onBack: () => void;
}
...
export default function PPOBPaketTelponReview({ formData, onConfirm, onBack }: PPOBPaketTelponReviewProps) {
    ...
    const handleSaveTransaction = async () => {
        ...
        try {
            const auditDocRef = await addDoc(collection(firestore, 'ppobTransactions'), {
                ...
                description: `Paket Telpon: ${formData.packageName}`,
            ...
            await runTransaction(firestore, async (transaction) => {
                ...
                transaction.set(debitTxRef, {
                    kasAccountId: sourcePPOBAccount.id, type: 'debit', name: `Beli ${formData.packageName}`, account: 'PPOB', date: nowISO, amount: costPrice, balanceBefore: currentSourcePPOBBalance, balanceAfter: currentSourcePPOBBalance - costPrice, category: 'ppob_purchase', deviceName, auditId
                });
                ...
                switch (paymentMethod) {
                    case 'Tunai':
                        ...
                        transaction.set(creditTunaiRef, {
                             kasAccountId: laciAccount.id, type: 'credit', name: `Bayar ${formData.packageName}`, account: 'Pelanggan', date: nowISO, amount: sellingPrice, balanceBefore: currentLaciBalance, balanceAfter: currentLaciBalance + sellingPrice, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        break;
                    case 'Transfer':
                        ...
                        transaction.set(creditTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar ${formData.packageName}`, account: 'Pelanggan', date: nowISO, amount: sellingPrice, balanceBefore: currentPaymentBalance, balanceAfter: currentPaymentBalance + sellingPrice, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        break;
                    case 'Split':
                        if (!laciAccountRef || !laciDoc || !laciDoc.exists()) throw new Error("Akun Laci tidak ditemukan.");
                        ...
                        transaction.set(creditSplitTunaiRef, {
                             kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Tunai ${formData.packageName}`, account: 'Pelanggan', date: nowISO, amount: splitTunaiAmount, balanceBefore: currentLaciSplitBalance, balanceAfter: currentLaciSplitBalance + splitTunaiAmount, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        ...
                        transaction.set(creditSplitTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Transfer ${formData.packageName}`, account: 'Pelanggan', date: nowISO, amount: splitTransferAmount, balanceBefore: currentPaymentSplitBalance, balanceAfter: currentPaymentSplitBalance + splitTransferAmount, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        break;
                }
            });

            toast({ title: "Sukses", description: "Transaksi Paket Telpon berhasil disimpan." });
            onConfirm();
            ...
            <ScrollArea className="flex-1 -mx-6 px-6">
                <div className="space-y-6 pt-4 pb-6">
                    <div className="space-y-2">
                        <h4 className="font-semibold text-lg">Detail Transaksi Paket Telpon</h4>
                        <div className="text-sm space-y-1 text-muted-foreground">
                            <p>Nama Paket: <strong>{formData.packageName}</strong></p>
