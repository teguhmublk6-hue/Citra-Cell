...
import type { PPOBWifiFormValues } from "@/lib/types";
...
interface PPOBWifiReviewProps {
    formData: PPOBWifiFormValues;
    onConfirm: () => void;
    onBack: () => void;
}
...
export default function PPOBWifiReview({ formData, onConfirm, onBack }: PPOBWifiReviewProps) {
    ...
    const handleSaveTransaction = async () => {
        ...
        setIsSaving(true);
        toast({ title: "Memproses...", description: "Menyimpan transaksi Wifi." });
        ...
        try {
            const auditDocRef = await addDoc(collection(firestore, 'ppobWifi'), {
                date: now,
            ...
            await runTransaction(firestore, async (transaction) => {
                ...
                transaction.set(debitTxRef, {
                    kasAccountId: sourcePPOBAccount.id, type: 'debit', name: `Bayar Tagihan Wifi`, account: formData.customerName, date: nowISO, amount: billAmount, balanceBefore: currentSourcePPOBBalance, balanceAfter: balanceAfterDebit, category: 'ppob_wifi_payment', deviceName, auditId
                });
                ...
                switch (paymentMethod) {
                    case 'Tunai':
                        ...
                        transaction.set(creditTunaiRef, {
                             kasAccountId: laciAccountId, type: 'credit', name: `Bayar Tagihan Wifi`, account: formData.customerName, date: nowISO, amount: totalAmount, balanceBefore: currentLaciBalance, balanceAfter: currentLaciBalance + totalAmount, category: 'ppob_wifi_payment', deviceName, auditId
                        });
                        break;
                    case 'Transfer':
                        ...
                        transaction.set(creditTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Tagihan Wifi`, account: formData.customerName, date: nowISO, amount: totalAmount, balanceBefore: currentPaymentAccBalance, balanceAfter: currentPaymentAccBalance + totalAmount, category: 'ppob_wifi_payment', deviceName, auditId
                        });
                        break;
                    case 'Split':
                        ...
                        transaction.set(creditSplitTunaiRef, {
                             kasAccountId: laciAccountId, type: 'credit', name: `Bayar Tunai Tagihan Wifi`, account: formData.customerName, date: nowISO, amount: splitTunaiAmount, balanceBefore: currentLaciBalance, balanceAfter: currentLaciBalance + splitTunaiAmount, category: 'ppob_wifi_payment', deviceName, auditId
                        });
                        ...
                        transaction.set(creditSplitTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Transfer Tagihan Wifi`, account: formData.customerName, date: nowISO, amount: splitTransferAmount, balanceBefore: currentPaymentAccBalance, balanceAfter: currentPaymentAccBalance + splitTransferAmount, category: 'ppob_wifi_payment', deviceName, auditId
                        });
                        break;
                }
            });
            ...
            <ScrollArea className="flex-1 -mx-6 px-6">
                <div className="space-y-6 pt-4 pb-6">
                    <div className="space-y-2">
                        <h4 className="font-semibold text-lg">Detail Tagihan Wifi</h4>
