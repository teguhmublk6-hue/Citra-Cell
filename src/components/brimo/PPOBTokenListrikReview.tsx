...
interface PPOBTokenListrikReviewProps {
    formData: PPOBTokenListrikFormValues;
    onConfirm: () => void;
    onBack: () => void;
}
...
export default function PPOBTokenListrikReview({ formData, onConfirm, onBack }: PPOBTokenListrikReviewProps) {
        ...
        try {
            ...
            await runTransaction(firestore, async (transaction) => {
                ...
                transaction.set(debitTxRef, {
                    kasAccountId: sourcePPOBAccount.id, type: 'debit', name: `Beli Token Listrik ${formData.denomination}`, account: formData.customerName, date: nowISO, amount: costPrice, balanceBefore: currentSourcePPOBBalance, balanceAfter: currentSourcePPOBBalance - costPrice, category: 'ppob_purchase', deviceName, auditId
                });
                ...
                switch (paymentMethod) {
                    case 'Tunai':
                        ...
                        transaction.set(creditTunaiRef, {
                             kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Token ${formData.denomination}`, account: formData.customerName, date: nowISO, amount: sellingPrice, balanceBefore: currentLaciBalance, balanceAfter: currentLaciBalance + sellingPrice, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        break;
                    case 'Transfer':
                        ...
                        transaction.set(creditTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Token ${formData.denomination}`, account: formData.customerName, date: nowISO, amount: sellingPrice, balanceBefore: currentPaymentBalance, balanceAfter: currentPaymentBalance + sellingPrice, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        break;
                    case 'Split':
                         if (!laciAccountRef || !laciDoc || !laciDoc.exists()) throw new Error("Akun Laci tidak ditemukan.");
                        ...
                        transaction.set(creditSplitTunaiRef, {
                             kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Tunai Token`, account: formData.customerName, date: nowISO, amount: splitTunaiAmount, balanceBefore: currentLaciSplitBalance, balanceAfter: currentLaciSplitBalance + splitTunaiAmount, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        ...
                        transaction.set(creditSplitTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Transfer Token`, account: formData.customerName, date: nowISO, amount: splitTransferAmount, balanceBefore: currentPaymentSplitBalance, balanceAfter: currentPaymentSplitBalance + splitTransferAmount, category: 'customer_payment_ppob', deviceName, auditId
                        });
                        break;
                }
            });
