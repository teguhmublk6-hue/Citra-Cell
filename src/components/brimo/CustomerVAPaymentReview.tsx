            ...
            await runTransaction(firestore, async (transaction) => {
                ...
                if (adminFee && adminFee > 0) {
                    ...
                    transaction.set(debitFeeRef, {
                        kasAccountId: sourceAccount.id, type: 'debit', name: `Biaya Admin VA an. ${formData.recipientName}`, account: 'Biaya VA', date: nowISO, amount: adminFee, balanceBefore: currentSourceBalance - paymentAmount, balanceAfter: currentSourceBalance - totalDebitFromSource, category: 'customer_va_payment_fee', deviceName, auditId
                    });
                }
                ...
                switch (paymentMethod) {
                    case 'Tunai':
                        ...
                        transaction.set(creditTunaiRef, {
                             kasAccountId: laciAccount.id, type: 'credit', name: `Bayar VA an. ${formData.recipientName}`, account: 'Pelanggan', date: nowISO, amount: totalPaymentByCustomer, balanceBefore: currentLaciBalance, balanceAfter: currentLaciBalance + totalPaymentByCustomer, category: 'customer_payment_va', deviceName, auditId
                        });
                        break;
                    case 'Transfer':
                        ...
                        transaction.set(creditTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar VA an. ${formData.recipientName}`, account: 'Pelanggan', date: nowISO, amount: totalPaymentByCustomer, balanceBefore: currentPaymentBalance, balanceAfter: currentPaymentBalance + totalPaymentByCustomer, category: 'customer_payment_va', deviceName, auditId
                        });
                        break;
                    case 'Split':
                        if (!laciAccountRef || !laciDoc || !laciDoc.exists()) throw new Error("Akun Laci tidak ditemukan.");
                        ...
                        transaction.set(creditSplitTunaiRef, {
                             kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Tunai VA an. ${formData.recipientName}`, account: 'Pelanggan', date: nowISO, amount: splitTunaiAmount, balanceBefore: currentLaciSplitBalance, balanceAfter: currentLaciSplitBalance + splitTunaiAmount, category: 'customer_payment_va', deviceName, auditId
                        });
                        ...
                        transaction.set(creditSplitTransferRef, {
                            kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Transfer VA an. ${formData.recipientName}`, account: 'Pelanggan', date: nowISO, amount: splitTransferAmount, balanceBefore: currentPaymentSplitBalance, balanceAfter: currentPaymentSplitBalance + splitTransferAmount, category: 'customer_payment_va', deviceName, auditId
                        });
                        break;
                }
            });
            ...
            onConfirm();
             ...
             if (!(error instanceof FirestorePermissionError)) {
                 const permissionError = new FirestorePermissionError({
                    path: 'customerVAPayments_transaction',
                    operation: 'write',
                    requestResourceData: { 
                        error: error.message,
                        formData,
