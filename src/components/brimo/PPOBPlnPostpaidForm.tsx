...
export default function PPOBPlnPostpaidForm({ onTransactionComplete, onDone }: PPOBPlnPostpaidFormProps) {
  ...
  const form = useForm<PPOBPlnPostpaidFormValues>({
    resolver: zodResolver(PPOBPlnPostpaidFormSchema),
    defaultValues: {
        sourcePPOBAccountId: '',
        customerName: '',
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, currentField: keyof typeof inputRefs, nextField?: keyof typeof inputRefs) => {
    if (e.key === 'Enter') {
      ...
      if (nextField && inputRefs[nextField]?.current) {
        ...
        // If there's no next field, maybe submit the form or do nothing
            ...
            switch (paymentMethod) {
                case 'Tunai':
                    ...
                    transaction.set(creditTunaiRef, {
                         kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Tagihan PLN`, account: values.customerName, date: nowISO, amount: totalAmount, balanceBefore: currentLaciBalance, balanceAfter: currentLaciBalance + totalAmount, category: 'ppob_pln_postpaid_payment', deviceName, auditId
                    });
                    break;
                case 'Transfer':
                    ...
                    transaction.set(creditTransferRef, {
                        kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Tagihan PLN`, account: values.customerName, date: nowISO, amount: totalAmount, balanceBefore: currentPaymentBalance, balanceAfter: currentPaymentBalance + totalAmount, category: 'ppob_pln_postpaid_payment', deviceName, auditId
                    });
                    break;
                case 'Split':
                    if (!laciAccountRef || !laciDoc || !laciDoc.exists()) throw new Error("Akun Laci tidak ditemukan.");
                    ...
                    transaction.set(creditSplitTunaiRef, {
                         kasAccountId: laciAccount.id, type: 'credit', name: `Bayar Tunai Tagihan PLN`, account: values.customerName, date: nowISO, amount: splitTunaiAmount, balanceBefore: currentLaciSplitBalance, balanceAfter: currentLaciSplitBalance + splitTunaiAmount, category: 'ppob_pln_postpaid_payment', deviceName, auditId
                    });
                    ...
                    transaction.set(creditSplitTransferRef, {
                        kasAccountId: paymentTransferAccount!.id, type: 'credit', name: `Bayar Transfer Tagihan PLN`, account: values.customerName, date: nowISO, amount: splitTransferAmount, balanceBefore: currentPaymentSplitBalance, balanceAfter: currentPaymentSplitBalance + splitTransferAmount, category: 'ppob_pln_postpaid_payment', deviceName, auditId
                    });
                    break;
            }
        });
